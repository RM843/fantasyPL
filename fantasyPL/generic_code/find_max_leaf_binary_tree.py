...
def mcts_playout(root, num_iter, num_rollout, exploration_weight,problem_obj):
    ...
    value = problem_obj.reward_function((problem_obj.rounds.start,root[1]))
    root_node = Node(node_id=root,value=value,terminal= problem_obj.rounds.stop == root[0])

    mcts = MCTS(exploration_weight=exploration_weight)
    while True:
        # we run MCTS simulation for many times
        for _ in range(num_iter):
            mcts.run(root_node, num_rollout=num_rollout)
